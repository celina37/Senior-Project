...
if(isset($_GET['id'])) {
    ...
    try {
     // Get the product_color_ids for the products with category_id = $category_id
$stmt = mysqli_prepare($conn, "SELECT id FROM product_color WHERE product_id IN (SELECT pID FROM products WHERE category_id = ?)");
...
while ($row = mysqli_fetch_assoc($result)) {
    ...
    mysqli_stmt_execute($stmt6);
        ...
        // Delete product_color related to the products with category_id = $category_id
        $stmt = mysqli_prepare($conn, "DELETE FROM product_color WHERE product_id IN (SELECT pID FROM products WHERE category_id = ?)");
        mysqli_stmt_bind_param($stmt, "i", $category_id);
        mysqli_stmt_execute($stmt);

        // Delete products with category_id = $category_id
        $stmt = mysqli_prepare($conn, "DELETE FROM products WHERE category_id = ?");
        mysqli_stmt_bind_param($stmt, "i", $category_id);
        mysqli_stmt_execute($stmt);

        // Delete subcategories with cat_id = $category_id
        $stmt = mysqli_prepare($conn, "DELETE FROM subcategories WHERE cat_id = ?");
        mysqli_stmt_bind_param($stmt, "i", $category_id);
        mysqli_stmt_execute($stmt);
        ...
        echo "Error deleting products, subcategories, and category: " . $e->getMessage();
    }
} else {
